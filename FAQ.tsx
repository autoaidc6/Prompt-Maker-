        ...
        {FAQ_DATA.map((item, index) => (
          <div key={index} className="border border-slate-800 rounded-xl overflow-hidden bg-slate-800/30">
            ...
            >
              ...
              {openIndex === index ? <ChevronUp size={20} className="text-indigo-400" /> : <ChevronDown size={20} className="text-slate-500" />}
            </button>
            <div
            ...
            >
              <div className="p-4 pt-0 text-slate-400 leading-relaxed text-sm">
                {item.answer}
              </div>
            </div>
          </div>
